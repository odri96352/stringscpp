...
void test_constructor_c_string_array(char* t, int size_t){
  std::cout << "Testing the constructor from an array of char from which we limit the size"<<std::endl;
  std::cout << "Should be: "<<std::endl;
...
void test_reserve_and_capacity(string t){
  ...
  int new_capacity=t.capacity()+3;
  t.reserve(new_capacity);
  std::cout<<"After increasing the capacity by 3, the string ";
  t.print();
  std::cout<<"Should takes up : "<<std::endl;
  std::cout<<"5 bytes"<<std::endl;
  std::cout<<"Takes up : "<<std::endl;
  std::cout<<t.capacity();
  std::cout<<" bytes."<<std::endl;
  int new_capacity_2=t.capacity()-2;
  t.reserve(new_capacity_2);
